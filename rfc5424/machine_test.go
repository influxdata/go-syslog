...
<2>1 - - - - - -`),
	...
	// Non existing date // (todo) > appending column to the parse timestamp error
	{
	...
	// (fixme) > hostname & c. can contain dashes (eg. nil value), it is correct?
}
...
func TestParse(t *testing.T) {
	for _, tc := range testCases {
		...
		t.Run(createName(tc.input), func(t *testing.T) {
			// t.Parallel() // (fixme) > re-enable

			fsm := NewMachine()
			msg, err := fsm.Parse(tc.input, nil)
			...
			if !tc.valid {
				...
				bestEffort := true
				m, e := fsm.Parse(tc.input, &bestEffort)
				assert.Equal(t, tc.partialValue, m)
				assert.EqualError(t, e, tc.errorString)
			}
			if tc.valid {
				assert.Nil(t, err)
